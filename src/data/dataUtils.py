...
class DataSample(Data):
  def __init__(self, path2data, speaker,
               ...
               view=None):
    self.view = view
    super().__init__(path2data=path2data, speaker=speaker,
                     modalities=modalities, fs_new=fs_new, split=split,
                     batch_size=batch_size, shuffle=shuffle, num_workers=num_workers,
                     window_hop=window_hop,
                     load_data=load_data,
                     style_iters=style_iters,
                     num_training_sample=num_training_sample,
                     sample_all_styles=sample_all_styles,
                     repeat_text=repeat_text,
                     quantile_sample=quantile_sample,
                     quantile_num_training_sample=quantile_num_training_sample)

  def get_tdt(self, x):
    return self.get_df_subset('interval_id', x).iloc[0].dataset
  
  def getPath2file(self, x):
    return (Path(self.view)/'keypoints'/self.get_tdt(x)/self.getSpeaker(x)/str(x)).as_posix() + '.h5'

  def get_minidata_list(self, intervals):
    new_intervals = list(filter(None, [interval_id if os.path.exists(self.getPath2file(interval_id)) else None for interval_id in intervals]))
    return [MiniData(self.getPath2file(interval_id), style=self.getStyle(interval_id), **self.minidataKwargs)
